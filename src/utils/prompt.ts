...
export function messagesFromLineage(
  ...
  for (let i = lineage.length - 1; i >= 0; i--) {
    ...
    if (node.data.staveNodeType === StaveNodeType.System) {
      messages.push({
        role: "system",
    ...
    if (node.data.staveNodeType === StaveNodeType.User) {
      messages.push({
        role: "user",
        content: node.data.text,
      });
